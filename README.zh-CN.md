...
推荐方式：
直接从 [Releases](https://github.com/cfsheep/codexdian/releases) 下载最新发布包。

1. 下载最新 release ZIP。
2. 在你的 vault 中创建目录：
   `.obsidian/plugins/codexdian`
3. 把 ZIP 里的 `codexdian` 文件夹解压到该目录，或者至少放入这 3 个文件：
   `manifest.json`、`main.js`、`styles.css`
4. 打开 Obsidian。
5. 在 `设置 -> 第三方插件` 里启用 `Codexdian`。

兜底方式：
如果你下载的是整个仓库 ZIP，也不必自己编译，只要把上面这 3 个文件放到正确目录即可。
